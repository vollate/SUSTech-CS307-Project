...
let newReplyAnonymous = false;
let newSecReplyAnonymous = false;
...
async function newReply() {
  ...
  config.body = JSON.stringify({
    type: "PostOp",
    content: ["AddReply", Parameters.post.post_id, newReplyContent, GlobalVariable.userName, newReplyAnonymous]
  })
...
async function newSecReply(replyId) {
  ...
  config.body = JSON.stringify({
    type: "PostOp",
    content: ["AddSecondReply", replyId, newSecReplyContent, GlobalVariable.userName, newSecReplyAnonymous]
  })
  return fetch(GlobalVariable.postURL, config)
      .then(res => res.json())
      .then(data => {
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="form-check">
        <input v-model="newReplyAnonymous" class="form-check-input" type="checkbox" value="" id="flexCheckDefault">
    ...
    <div class="row">
      ...
      <div v-for="reply of post.replies">
        <p class="row reply-info " v-show="reply.anonymity"> Author: ANONYMITY</p>
        <p class="row reply-info " v-show="!reply.anonymity"> Author: {{ reply.author_name }}</p>
        ...
        <div class="row">
          <input v-model="newSecReplyContent" type="text" class="form-control col"
                 id="floatingPassword"
                 placeholder="New Reply">
          <button @click="newSecReply(reply.reply_id)" class="btn col-1 btn">Submit</button>
        </div>
        <input v-model="newSecReplyAnonymous" class="form-check-input" type="checkbox" value="" id="flexCheckDefault">
        <label class="form-check-label" for="flexCheckDefault">
          anonymous
        </label>        <hr/>
        ...
        <div class="card">
          <div class="card-body">
            <div class="card-title" v-for="secReply of reply.secReplies">
              <div class="row">
                <p class="sec-reply-title col" v-show="secReply.anonymity">Author: ANONYMITY</p>
                <p class="sec-reply-title col" v-show="!secReply.anonymity">Author: {{ secReply.author_name }}</p>
